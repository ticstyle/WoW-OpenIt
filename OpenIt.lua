...
local function CleanTooltipText(text)
    ...
    return text:gsub("\194\160", " "):gsub("\160", " "):gsub("|[cC]%x%x%x%x%x%x%x%x", ""):gsub("|[cC]%x%x%x%x%x%x", ""):gsub("|r", ""):gsub("|T.-|t", "")
end
...
local function HasUnmetRequirements(bag, slot)
    ...
    for _, lineData in ipairs(tooltipData.lines) do
        ...
        if (lineData.leftColor and IsRedColor(lineData.leftColor))
...
local function IsItemOpenable(bag, slot, info)
    ...
    if classID then
        if classID == Enum.ItemClass.Armor
    ...
    if classID == Enum.ItemClass.Container or classID == Enum.ItemClass.Miscellaneous or classID == Enum.ItemClass.Quest then
        return true
    end
